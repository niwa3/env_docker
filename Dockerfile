...
RUN yum install -y lua-devel ncurses-devel perl-ExtUtils-Embed tmux python2 python-devel
...
RUN make distclean
RUN ./configure --prefix=/usr/local/ --with-features=huge --enable-multibyte --enable-luainterp --enable-pythoninterp --with-python-command=/bin/python2 --with-python-config-dir=/lib64/python2.7/config/ --enable-cscope --enable-fail-if-missing
RUN make
RUN make install
